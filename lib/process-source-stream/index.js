...
ProcessSourceStream.prototype.resolveEntry = function (entry) {
	...
	resolver(entry, this.options, function (err, entry) {
		...
		// remove a reference as it will be meaningless and was only useful for the first attempt
		// at resolving the entry
		delete entry.from;
			...
			if (entry.requires.length) {
				...
				entry.requires = entry.requires.map(function (dep) {
					var
						...
						next = {path: dep, base: base, external: !! entry.external, from: entry.fullpath},
					...
					next.name = name;
					// the reason this works is because any lib reaching into another lib won't
					// be using a relative path thus the lib here would be overwritten as expected
					// with the correct value, for all other cases this will already be correct
					if (next.external) next.lib = entry.lib;
