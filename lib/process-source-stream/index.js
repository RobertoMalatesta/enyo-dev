...
ProcessSourceStream.prototype.resolveEntry = function (entry) {
	...
	if (entry.external) {
		if (this._externals[entry.name]) {
			return done();
		} else if (opts.skipExternals && opts.skipExternals.length) {
			var lib = entry.lib || entry.name.split('/').shift();
			if (opts.skipExternals.indexOf(lib) > -1) {
				console.log('skipping "' + entry.name + '" because it is in a skipped library "' +
					lib + '"');
				return done();
			}
		}
