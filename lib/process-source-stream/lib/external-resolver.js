...
function LOAD_EXTERNAL (x, paths, done) {
	...
	if (PATHS[search]) {
		console.log('YO', search, PATHS[search], name, parts);
		// we've seen a request for this module before and have already resolved where it is
		if (!parts.length) return resolve('.', search, null, done);
		else return resolve('./' + parts.join('/'), PATHS[search], null, done);
	}
