...
ProcessSourceStream.prototype.resolveEntry = function (entry) {
	...
	entry.name = entry.name || nameResolver(entry, this.options, this);
		...
		if (!entry.cached) {
			if (entry.fullpath) {
				if (stream._fullpaths[entry.fullpath]) {
					...
					log.debug('module %s was resolved, but apparently had already been resolved as %s (from %s)',
						entry.name, stream._fullpaths[entry.fullpath].name, entry.from);
			...
			log.debug('resolved module %s at %s', entry.name, entry.fullpath);
		...
		if (entry.requires && entry.requires.length) {
			...
			entry.requires = entry.requires.map(function (dep) {
				var
					...
					name = nameResolver(next, stream.options, stream),
				...
				if (stream._resolvingNames[name]) {
					log.debug('dependency %s is already being resolved', name);
					return ret;
				}
