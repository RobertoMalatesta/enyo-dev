...
ProcessAssetsStream.prototype._transform = function (bundle, nil, next) {
	...
	if (bundle.cached) {
		log.debug('skipping processing of assets for cached bundle %s', bundle.name);
		return next(null, bundle);
	}
	
	if (opts.skipExternals && opts.skipExternals.indexOf(bundle.name) > -1) {
		log.debug('skipping asset processing for bundle %s', bundle.name);
		return next(null, bundle);
	}
